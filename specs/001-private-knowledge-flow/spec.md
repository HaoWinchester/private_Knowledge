...
2. **Given** 知识包含客户资料、合同条款、源代码、报价信息或敏感人员信息，**When** 资料进入入库、检索或问答流程，**Then** 系统按规则触发脱敏、审批、限制访问或禁止复用。
...
3. **Given** 上层应用请求高敏知识，**When** 用户或应用缺少必要授权，**Then** 系统拒绝请求并记录拒绝原因。
...
- **FR-016**: 系统 MUST 在展示、引用、导出、下载或向上层应用提供知识前，校验角色、部门、项目、密级和显式授权规则。
...
- **FR-026**: 系统 MUST 支持一期轻量接入统一身份、文档目录、共享文件夹、项目资料样例和消息提醒，且不替代原系统正式流程。
